...
| **Stage 6** | ✅ Complete | System Integration Adapters & Payload Contracts (doc 06 & doc 10): `pos-adapter.ts` (internal Postgres `stock_ledger` balance sync & electronic shelf tag updates; no external POS software connected yet), `supplier-adapter.ts` (Purchase Order JSON payload formatting & HTTP 201 dispatch receipt generator), `notification-adapter.ts` (Webhook alerts with deep-links `mrmart://actions/...`; no live Twilio/Meta WhatsApp credentials configured yet), accounting CSV/JSON file export generator, and 3/3 verification tests passing (`verify-stage6.ts`). |
...
| **Stage 8** | ✅ Complete (State Machine & Math Real) | SaaS Billing & Multi-Tenant Licensing (doc 09 & doc 10 Stage 8): Plan tiers (`trial`, `starter`, `growth`, `pro`), state machine (`active`, `past_due`, `degraded`, `cancelled`), `014_subscriptions_billing.sql` migration, degraded account `403 Forbidden` mutation block (`approve`/`reject` blocked while read-only cockpit remains `200 OK`), deterministic proration math (`unused_credit`, `new_plan_remaining_cost`, `prorated_charge`), `GET /api/billing/subscription` & `POST /api/billing/upgrade` endpoints, and 4/4 verification tests passing (`verify-stage8.ts`). **Explicit Open Item:** "Razorpay integration: stubbed locally, no live test-mode credentials configured yet — real integration blocked pending RAZORPAY_KEY_ID/RAZORPAY_KEY_SECRET." State machine and proration math are real, actual money movement is not, and both are explicitly visible. |
| **Stage 9** | ✅ Complete (Full Master QA Pass) | Full Test Pass & Pilot Readiness (doc 07 & doc 10): 30/30 unit tests passing on pure formula module (`npm run test --workspace=packages/mcp-server`), 8/8 automations & alerts fully verified across Happy Path, Reject Path, Duplicate-Prevention, and 3-attempt BullMQ Retries + DLQ state transitions (`verify-stage9-qa-suite.ts`), 50-SKU Queue Burst scenario drained cleanly off real Redis Cloud, Redis Outage graceful read degradation verified, Staff-role restriction & API token security re-confirmed, 3-Second rule & 56dp One-Tap UI audit passed, real reject-rate tracking query operational, and line-by-line release checklist walkthrough completed (39/39 tests passing on real Neon Postgres + real Redis Cloud). **Explicit Named Blocker for Real Pilot Launch:** "Razorpay integration: stubbed locally (`order_razorpay_stub_...`), no live test-mode credentials configured yet — real integration blocked pending `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET`. All database, queue, formula, state machine, and UI workflows are 100% real and verified; actual money movement is currently stubbed pending Razorpay credentials." |
